...
- (void)instanceDevice{
    ...
    CGFloat scanViewW = SCREEN_WIDTH * (214.00 / 375);
    CGFloat scanViewH = scanViewW;
    
    CGFloat x = ((ScreenHeight - NAVIGATIONBAR_VER_HEIGHT - scanViewH) * 0.5) / ScreenHeight;
    CGFloat y = ((ScreenWidth - scanViewW) * 0.5) / ScreenWidth;
    CGFloat width = scanViewH / ScreenHeight;
    CGFloat height = scanViewW / ScreenWidth;
...
- (void)setOverlayPickerView
{
    ...
    //扫描框图片的拉伸，拉伸中间一块区域
    UIImage *scanImage = [UIImage imageNamed:@"scanCode_blue"];
    ...
    line.tag = line_tag;
    line.image = [UIImage imageNamed:@"scanLine_blue"];
...
- (void)layoutSubviews{
    ...
    if(self.scanW){
        ...
        self.scanW = SCREEN_WIDTH * (214.00 / 375);
    }
    ...
    //最上部view
    _upView.frame = CGRectMake((ScreenWidth - scanViewW) * 0.5, 0, scanViewW, (self.frame.size.height - scanViewW - NAVIGATIONBAR_VER_HEIGHT)/2);
    //底部view
