...
#define MAX_IMAGEPIX 300   //分享图片控制 不要修改
#define MAX_BIGIMAGE 500

#define THUMBNAIL_IMAGE_WIDTH   ((SCREEN_WIDTH - 138.0f)/3.0f*2.0f);
...
+ (UIImage *)imageWithCompressBigImage:(UIImage *)image
{
    CGSize imageSize = image.size;
    CGFloat width = imageSize.width;
    CGFloat height = imageSize.height;
    
    if(width <= MAX_BIGIMAGE && height <= MAX_BIGIMAGE){
        return image;//不需要压缩
    }
    ...
    UIImage *newImage = nil;
    CGFloat widthFactor = MAX_BIGIMAGE /
    width;
    CGFloat heightFactor = MAX_BIGIMAGE / height;
    CGFloat compressFactor = 0.0;
...
+(NSString *)doctorGroupThumbnailImageParam {
    ...
    return [NSString stringWithFormat:@"imageView2/2/w/%d/h/%d", (int)ceilf(width), (int)ceilf(width)];
}

//由UIView生成image
+ (UIImage *)imageFromView:(UIView *)v rect:(CGRect)rect {
    CGFloat scale = [UIScreen mainScreen].scale;
    UIGraphicsBeginImageContextWithOptions(v.frame.size, NO, scale);  //NO，YES控制是否透明
